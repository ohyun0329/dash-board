...
# 2. 구글 시트 주소 (URL 끝부분을 export?format=xlsx로 수정해야 파이썬이 읽을 수 있습니다)
SHEET_URLS = {
...
def load_data(url, team_name):
    try:
        ...
        def clean_section(d, col_name):
            if d.empty: return d
            stops = ["화주", "본선", "구분", "내용", "입항", "인원", "nan", "None", "작업구분"]
        ...
        if idx_w is not None:
            ...
            if "중량" in team_name or "물류" in team_name:
...
with t1:
    ...
    if not all_att.empty:
        m_c = all_att['관리자 현황'].apply(count_names).sum()
        f_c = all_att['기사/다기능/선원 현황'].apply(count_names).sum()
        st.markdown(f"""<div class="total-card"><h3>📢 경남지사 금일 투입 총원: {m_count + f_count}명</h3>
                    <p>관리자: {m_c}명 | 기사/다기능/선원: {f_c}명</p></div>""", unsafe_allow_html=True)

    # 1. 금일 작업
    st.subheader("1. 금일 작업")
    if not all_w.empty:
        summary_w = all_w.groupby('팀명').agg(list).reset_index()
        html_w = "<table class='merged-table'><tr><th class='first-col'>팀명</th><th>화주</th><th>작업내용</th><th style='width:150px;'>투입인원</th><th style='width:150px;'>비고</th></tr>"
